...
__global__ void fill_gpu(Matrix h, Matrix d, char seqA[], char seqB[],
  ...
  if (i < h.height && j < h.width) { // Ensure thread within matrix bounds
    ...
    int old_max_score = atomicMax(&max_id_val[1], max_score);
    if (old_max_score < max_score) {
      atomicExch(&max_id_val[0], id);
    }
...
void smith_water_gpu(Matrix h, Matrix d, char seqA[], char seqB[]) {
  ...
  for (int i = 1; i <= ((A_LEN + 1) + (B_LEN + 1) - 1); i++) {
    ...
    // Launch the kernel with a 1D grid and block configuration
    int blks = 1;
    ...
    fprintf(stderr, "Lỗi sau cuộc gọi kernel: %s\n", cudaGetErrorString(cudaStatus));
    // Thực hiện xử lý lỗi hoặc return
}
